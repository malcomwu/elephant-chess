...
// Ref: https://www.w3.org/TR/2011/WD-html-markup-20110113/syntax.html
// A complete list of the void elements in HTML:
const IS_HTML_VOID = toSet(['area', 'base', 'br', 'col', 'command', 'embed', 'hr', 'img', 'input', 'keygen', 'link', 'meta', 'param', 'source', 'track', 'wbr'])
...
el.create = (elName, attrs, content) => {
  if (typeof elName === 'object') return new Element(elName).create()
  return new Element(el(elName, attrs, content)).create()
}

// The el() returns a plain object but not Element, unknown..
